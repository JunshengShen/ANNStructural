			...
			for z in range(128):
				if points[x][y][z]==0:
					for i in range(len(centers)):
						if distance([x,y,z],centers[i]) <= (potentials[i]/70):
							points[x][y][z]=1
	...
	for x in range(128):
		for y in range(128):
			for z in range(128):
				if points[x][y][z]==0:
					for i in range(len(centers)):
						if abs(x-centers[i][0]) <= (potentials[i]/70) and abs(y-centers[i][1]) <= (potentials[i]/70) and abs(z-centers[i][2]) <= (potentials[i]/70):
		...
		for y in range(128):
			for z in range(128):
				if points[x][y][z]==1:
					count+=1
	print(count)
	k,centers,lable = multiCluster([31,32] , points , 1)
